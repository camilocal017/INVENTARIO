...
import { MoreHorizontal, Minus, Plus, ShoppingCart, Trash2 } from "lucide-react";
...
// 👇 NUEVO: importamos el store para eliminar
import { useInventoryStore } from "@/hooks/use-inventory-store";

type ProductTableProps = {
  products: Product[];
  onStockChange: (productId: string, newStock: number) => void;
  onRecordSale: (productId: string, quantity: number) => {
    success: boolean;
    message: string;
  };
};
...
const { removeProduct } = useInventoryStore();
  ...
  const handleAdjustStock = () => {
    if (selectedProduct) {
      ...
      toast({
        title: "Success",
        description: `Stock for ${selectedProduct.name} updated to ${quantity}.`,
      });
      handleCloseDialog();
    }
  };
    ...
    <>
                  ...
                  <TableCell className="text-right">
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end">
                        <DropdownMenuItem
                          onClick={() => handleOpenDialog("record-sale", product)}
                        >
                          <ShoppingCart className="mr-2 h-4 w-4" />
                          Registrar venta
                        </DropdownMenuItem>
                        <DropdownMenuItem
                          onClick={() => handleOpenDialog("adjust-stock", product)}
                        >
      ...
      <Dialog open={!!activeDialog} onOpenChange={handleCloseDialog}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
              {activeDialog === "adjust-stock" ? "Adjust Stock" : "Record Sale"}{" "}
